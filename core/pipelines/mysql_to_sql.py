...
from config.credentials import MYSQL, SQL_SERVER
from core.reports.google_sheets import GoogleSheetsReporter
from core.validator import aplicar_regras_central
from core.connectors import MySQLConnector, SQLServerConnector
...
class MySQLToSQLPipeline:
    def __init__(self, json_file, source=None, target=None):
        self.source = source or MySQLConnector(config=MYSQL)
    ...
    def get_database_names(self):
        origem = getattr(self.source, 'database', 'Origem Desconhecida')
        destino = getattr(self.target, 'database', 'Destino Desconhecida')
        return origem, destino


def executar(json_file):
    pipeline = MySQLToSQLPipeline(json_file)
